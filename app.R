                      ...
                      sidebarPanel(
                        ...
                        selectizeInput(
                          ...
                          options = list(create = TRUE,
                                         render = I("
                          {
                            item: function(item, escape) { return '<div><span style=\"font-weight: bold; color:' + escape(item.label) + ';\">' + escape(item.label) + '</span></div>'; },
                            option: function(item, escape) { return '<div><span style=\"font-weight: bold; color:' + escape(item.label) + ';\">' + escape(item.label) + '</span></div>'; }
                          }"))
                        ),
                        # style=\"background-color:' + item.label + ';\" # for changing the background instead of text colour (paste into first div)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$parameters_input, {
    ...
    # update sample renaming
    sample_names_table <<- parameters %>%
      filter(parameter %in% c("original_sample_name", "new_sample_name", "condition")) %>%
      mutate(parameter = case_when(parameter == "original_sample_name" ~ "sample_name",
                                   parameter == "condition" ~ "Condition",
                                   TRUE ~ parameter)) %>%
      pivot_wider(names_from = parameter, values_from = value) %>%
      mutate(across(everything(), ~str_split(.x, pattern = ","))) %>%
      unnest(c(sample_name, new_sample_name, Condition))
    
    # update colour
    updateSelectizeInput(session, "colour", selected = str_split(parameters[parameters$parameter == "colours",]$value, pattern = ",")[[1]],
                         choices = unique(c(protti_colours, str_split(parameters[parameters$parameter == "colours",]$value, pattern = ",")[[1]])))
    
    # update plot parameters
    updateNumericInput(session, "plot_height", value = as.numeric(parameters[parameters$parameter == "plot_height",]$value))
    updateNumericInput(session, "plot_width", value = as.numeric(parameters[parameters$parameter == "plot_width",]$value))
  })
  ...
  parameters_download <- reactive({
    ...
    parameters <<- parameters %>% 
      mutate(value = ifelse(parameter == "best_metal_modes", best_metal_mode_string, value),
             ...
             value = ifelse(parameter == "colours", paste0(input$colour, collapse = ","), value),
             value = ifelse(parameter == "plot_height", input$plot_height, value),
             value = ifelse(parameter == "plot_width", input$plot_width, value)
      ) %>% 
      filter(!parameter %in% c("dilution_factor", "dilution_factor_samples", "weight", 
  ...
  calculate_result <- reactive({
    ...
    if(is.null(clean_samples()) | nrow(sample_properties_df) == 0){
      ...
      if (is.null(sample_names_table) || 
          (all(sample_names_table$sample_name == sample_names_table$new_sample_name) & 
           all(sample_names_table$Condition == ""))){
        # prevent overwriting table if it actually already contains information e.g. when provided with parameter file or when switching tabs
